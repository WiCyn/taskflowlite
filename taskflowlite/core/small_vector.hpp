...
namespace tfl { namespace detail {
...
*/
inline uint64_t NextCapacity(uint64_t A) noexcept {
    A |= (A >> 1);
    A |= (A >> 2);
    A |= (A >> 4);
    A |= (A >> 8);
...
protected:
    SmallVectorBase(void *FirstEl, size_t Size) noexcept
...
public:
  ...
  */
  ...
  @private
  */
    size_t capacity_in_bytes() const noexcept {
        return size_t((char*)CapacityX - (char*)BeginX);
    }

    bool empty() const noexcept { return BeginX == EndX; }
};
...
protected:
    SmallVectorTemplateCommon(size_t Size) noexcept : SmallVectorBase(&FirstEl, Size) {}
    ...
    void resetToSmall() noexcept {
        BeginX = EndX = CapacityX = &FirstEl;
    }

    void setEnd(T *P) noexcept { this->EndX = P; }
...
public:
    ...
    inline iterator begin() noexcept { return (iterator)this->BeginX; }
    inline const_iterator begin() const noexcept { return (const_iterator)this->BeginX; }
    inline iterator end() noexcept { return (iterator)this->EndX; }
    inline const_iterator end() const noexcept { return (const_iterator)this->EndX; }

protected:

    iterator capacity_ptr() noexcept { return (iterator)this->CapacityX; }
    const_iterator capacity_ptr() const noexcept { return (const_iterator)this->CapacityX;}
...
public:
    ...
    inline size_type size() const noexcept { return end()-begin(); }
    inline size_type max_size() const noexcept { return size_type(-1) / sizeof(T); }

    /// Return the total number of elements in the currently allocated buffer.
    size_t capacity() const noexcept { return capacity_ptr() - begin(); }
...
protected:
    SmallVectorTemplateBase(size_t Size) noexcept : SmallVectorTemplateCommon<T>(Size) {}

    static void destroy_range(T *S, T *E) noexcept {
        while (S != E) {
            --E;
            E->~T();
        }
...
protected:
    ...
    static void destroy_range(T *, T *) noexcept {}
...
protected:
    ...
    explicit SmallVectorImpl(unsigned N) noexcept
...
public:
    ...
    iterator erase(const_iterator CI) noexcept {
    ...
    iterator erase(const_iterator CS, const_iterator CE) noexcept {
        // Just cast away constness because this is a non-const member function.
    ...
    bool operator==(const SmallVectorImpl &RHS) const noexcept {
...
public:
  ...
  */
   ...
   */
    ...
    //explicit SmallVector(const tfl::iterator_range<RangeTy> &R)
    //    : SmallVectorImpl<T>(N) {
  ...
  */
    SmallVector(SmallVectorImpl<T> &&RHS) noexcept : SmallVectorImpl<T>(N) {
        if (!RHS.empty())
            SmallVectorImpl<T>::operator=(::std::move(RHS));
    }
...
*/
template<typename T, unsigned N>
inline size_t capacity_in_bytes(const SmallVector<T, N> &X) noexcept {
...
template<typename T>
inline void
swap(tfl::SmallVectorImpl<T> &LHS, tfl::SmallVectorImpl<T> &RHS) {
    LHS.swap(RHS);
}
...
template<typename T, unsigned N>
inline void
swap(tfl::SmallVector<T, N> &LHS, tfl::SmallVector<T, N> &RHS) {
    LHS.swap(RHS);
}
